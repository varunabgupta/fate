...
```{r}
fourg <- Raw[celltypes == "4G",]
fourgf <- Raw[celltypes == "4GF",]
...
non_assignment_error <- (sum(erythroid_distances[assignments == "4GF"]) + sum(endothelial_distances[assignments == "4G"]))/nrow(Raw)
...
ggbiplot(pr.raw, obs.scale = 1, var.scale = 1, scale = 1, groups = assignments, ellipse = TRUE, circle = TRUE)
ggbiplot(pr.raw, obs.scale = 1, var.scale = 1, scale = 1, groups = total[, c("Fate")], ellipse = TRUE, circle = TRUE)
